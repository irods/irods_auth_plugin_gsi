...
// not need to support
irods::error gsi_auth_agent_verify(irods::plugin_context&, const char*, const char*, const char*) {
    return SUCCESS();
...
irods::auth* plugin_factory(
    ...
    irods::auth* result = nullptr;
    if ( ( ret = ASSERT_ERROR( gsi != NULL, SYS_MALLOC_ERR, "Failed to allocate a gsi plugin: \"%s\".",
                               _inst_name.c_str() ) ).ok() ) {

        gsi->add_operation( irods::AUTH_ESTABLISH_CONTEXT, std::function<irods::error(irods::plugin_context&)>(gsi_auth_establish_context) );
        gsi->add_operation<rcComm_t*,const char*>( irods::AUTH_CLIENT_START, std::function<irods::error(irods::plugin_context&,rcComm_t*,const char*)>(gsi_auth_client_start));
        gsi->add_operation<rcComm_t*>( irods::AUTH_CLIENT_AUTH_REQUEST, std::function<irods::error(irods::plugin_context&,rcComm_t*)>(gsi_auth_client_request));
        gsi->add_operation<rcComm_t*>( irods::AUTH_CLIENT_AUTH_RESPONSE, std::function<irods::error(irods::plugin_context&,rcComm_t*)>(gsi_auth_client_response));
#ifdef RODS_SERVER
        gsi->add_operation<const char*>( irods::AUTH_AGENT_START, std::function<irods::error(irods::plugin_context&,const char*)>(gsi_auth_agent_start));
        gsi->add_operation( irods::AUTH_AGENT_AUTH_REQUEST, std::function<irods::error(irods::plugin_context&)>(gsi_auth_agent_request));
        gsi->add_operation<authResponseInp_t*>( irods::AUTH_AGENT_AUTH_RESPONSE, std::function<irods::error(irods::plugin_context&,authResponseInp_t*)>(gsi_auth_agent_response) );
        gsi->add_operation<const char*,const char*,const char*>( irods::AUTH_AGENT_AUTH_VERIFY,  std::function<irods::error(irods::plugin_context&,const char*,const char*,const char*)>(gsi_auth_agent_verify));
#endif
        result = gsi;
    } else {
        irods::log(ret);
